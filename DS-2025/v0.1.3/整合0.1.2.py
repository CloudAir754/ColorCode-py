...
# 【未完成】4. 颜色管理过程，使用边缘处理结果；并按照顺序依次读取；
...
class ColorCodeDetector:
    def __init__(self, image_path):
        ...
        self.min_screen_coef =  8# 最小有效轮廓占总图像的1/x 8
    ...
    def classify_color(self,color):
        ...
        """
        # 定义 HSV 边界值
        RED_LOWER_HSV = (0, 50, 50)  # 红色的 HSV 下界
        RED_UPPER_HSV = (10, 255, 255)  # 红色的 HSV 上界
        GREEN_LOWER_HSV = (35, 50, 50)  # 绿色的 HSV 下界
        GREEN_UPPER_HSV = (85, 255, 255)  # 绿色的 HSV 上界
        BLUE_LOWER_HSV = (100, 50, 50)  # 蓝色的 HSV 下界
        BLUE_UPPER_HSV = (130, 255, 255)  # 蓝色的 HSV 上界
        BLACK_UPPER_HSV = (180, 255, 30)  # 黑色的 HSV 上界（V 值较低）

        # 定义 RGB 边界值
        RED_LOWER_RGB = (0, 0, 100)  # 红色的 RGB 下界
        RED_UPPER_RGB = (50, 50, 255)  # 红色的 RGB 上界
        GREEN_LOWER_RGB = (0, 100, 0)  # 绿色的 RGB 下界
        GREEN_UPPER_RGB = (50, 255, 50)  # 绿色的 RGB 上界
        BLUE_LOWER_RGB = (100, 0, 0)  # 蓝色的 RGB 下界
        BLUE_UPPER_RGB = (255, 50, 50)  # 蓝色的 RGB 上界
        BLACK_UPPER_RGB = (30, 30, 30)  # 黑色的 RGB 上界

        if self.show_hsv:
            # HSV 模式
            h, s, v = color
            if RED_LOWER_HSV[0] <= h <= RED_UPPER_HSV[0] and RED_LOWER_HSV[1] <= s <= RED_UPPER_HSV[1] and RED_LOWER_HSV[2] <= v <= RED_UPPER_HSV[2]:
                return "RED"
            elif GREEN_LOWER_HSV[0] <= h <= GREEN_UPPER_HSV[0] and GREEN_LOWER_HSV[1] <= s <= GREEN_UPPER_HSV[1] and GREEN_LOWER_HSV[2] <= v <= GREEN_UPPER_HSV[2]:
                return "GREEN"
            elif BLUE_LOWER_HSV[0] <= h <= BLUE_UPPER_HSV[0] and BLUE_LOWER_HSV[1] <= s <= BLUE_UPPER_HSV[1] and BLUE_LOWER_HSV[2] <= v <= BLUE_UPPER_HSV[2]:
                return "BLUE"
            elif v <= BLACK_UPPER_HSV[2]:  # 黑色主要看 V 值
                return "BLACK"
            else:
                return "NaN"
        else:
            # RGB 模式
            b, g, r = color
            if RED_LOWER_RGB[2] <= r <= RED_UPPER_RGB[2] and RED_LOWER_RGB[1] >= g >= RED_UPPER_RGB[1] and RED_LOWER_RGB[0] >= b >= RED_UPPER_RGB[0]:
                return "红色"
            elif GREEN_LOWER_RGB[1] <= g <= GREEN_UPPER_RGB[1] and GREEN_LOWER_RGB[2] >= r >= GREEN_UPPER_RGB[2] and GREEN_LOWER_RGB[0] >= b >= GREEN_UPPER_RGB[0]:
                return "绿色"
            elif BLUE_LOWER_RGB[0] <= b <= BLUE_UPPER_RGB[0] and BLUE_LOWER_RGB[1] >= g >= BLUE_UPPER_RGB[1] and BLUE_LOWER_RGB[2] >= r >= BLUE_UPPER_RGB[2]:
                return "蓝色"
            elif r <= BLACK_UPPER_RGB[2] and g <= BLACK_UPPER_RGB[2] and b <= BLACK_UPPER_RGB[2]:  # 黑色主要看 RGB 值
                return "黑色"
            else:
                return "未知颜色"
